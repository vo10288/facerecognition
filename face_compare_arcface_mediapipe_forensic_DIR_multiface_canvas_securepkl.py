...
def make_index_sha256_path(pkl_path: Path) -> Path:
    return pkl_path.with_suffix(pkl_path.suffix + ".sha256")


def make_index_manifest_path(pkl_path: Path) -> Path:
    return pkl_path.with_suffix(pkl_path.suffix + ".manifest.json")
...
class FaceCompareDirectoriesApp:
    ...
    def _save_index_pkl(self, dir_path: str, items: list[FaceResult]) -> Path:
        ...
        sha_path = make_index_sha256_path(pkl_path)
        manifest_path = make_index_manifest_path(pkl_path)
        ...
        for item in items:
            ...
            source_files[item.source_path] = item.sha256
...
", encoding="utf-8")
        ...
        manifest = {
            ...
            "index_for_directory": dir_path,
            "index_filename": pkl_path.name,
            "item_count": len(serializable),
            "pkl_sha256": pkl_sha256,
            "source_files": [
                {"path": path, "sha256": sha}
                for path, sha in sorted(source_files.items(), key=lambda x: x[0].lower())
            ],
        }
    ...
    def _load_index_pkl(self, dir_path: str) -> Optional[list[FaceResult]]:
        ...
        if not sha_path.exists() or not manifest_path.exists():
            self.cache_warnings.append(
                f"Cache PKL non verificabile per {pkl_path.name}: file .sha256 o manifest mancanti. Reindicizzazione richiesta."
            )
            return None
        ...
        try:
            ...
            self.cache_warnings.append(f"Impossibile leggere {sha_path.name}: {exc}")
            return None

        current_sha = file_sha256(str(pkl_path))
        if stored_sha != current_sha:
            self.cache_warnings.append(
                f"Integrità PKL fallita per {pkl_path.name}: SHA256 non corrispondente. Reindicizzazione richiesta."
            )
            return None
        ...
        try:
            ...
            self.cache_warnings.append(f"Manifest PKL non leggibile per {pkl_path.name}: {exc}")
            return None

        if manifest.get("schema_version") != PKL_CACHE_SCHEMA_VERSION:
            self.cache_warnings.append(
                f"Schema cache non compatibile per {pkl_path.name}. Reindicizzazione richiesta."
            )
            return None

        if manifest.get("pkl_sha256") != current_sha:
            self.cache_warnings.append(
                f"Manifest incoerente per {pkl_path.name}: hash manifest diverso dal file PKL."
            )
            return None
        ...
        for entry in manifest.get("source_files", []):
            ...
            if not src_path or not expected_sha:
                self.cache_warnings.append(
                    f"Manifest incompleto per {pkl_path.name}. Reindicizzazione richiesta."
                )
                return None
            if not os.path.exists(src_path):
                self.cache_warnings.append(
                    f"File sorgente mancante per cache {pkl_path.name}: {src_path}"
                )
                return None
            actual_sha = file_sha256(src_path)
            if actual_sha != expected_sha:
                self.cache_warnings.append(
                    f"File sorgente modificato per cache {pkl_path.name}: {src_path}. Reindicizzazione richiesta."
                )
                return None
        ...
        try:
            ...
            self.cache_warnings.append(f"Impossibile deserializzare {pkl_path.name}: {exc}")
            return None
    ...
    def index_both(self) -> None:
        try:
            ...
            txt.append("Sicurezza PKL: SHA256 + manifest JSON + verifica file sorgente")
            if self.cache_warnings:
                txt.append("")
                txt.append("Avvisi cache PKL:")
                txt.extend([f"- {w}" for w in self.cache_warnings])
            self._set_metrics("\n".join(txt))
